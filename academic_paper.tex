...
\subsection{Sparse discovery via genetic search (current implementation)}
...
y \approx \Theta \xi,
\end{equation*}
but rather than estimating $\xi$ via a regression solver, the current Rust implementation performs a lightweight genetic search over \emph{sparse} coefficient vectors:
\begin{itemize}
\item Each candidate ``equation'' is a short list of $(\text{feature}, \text{coefficient})$ terms with a maximum term budget $K$.
\item Fitness is mean squared error (MSE) on $y$, optionally multiplied by a parsimony penalty increasing with the number of terms.
\item We iterate: score a population, keep the best survivors, then mutate (perturb coefficients; add/remove terms) to repopulate.
\item We retain the top-3 equations per component and then combine component-wise equations into vector candidates that are evaluated by forward rollout (next subsection).
\end{itemize}

Related work: A natural alternative is sparse regression (SINDy/STLS)\cite{Brunton2016} or LASSO-style solvers\cite{Tibshirani1996} to estimate $\xi$ for a fixed library. These are not yet implemented in the current Rust codebase; the data contract (header-based CSV plus sidecar metadata) is designed to support them.
...
\item Discovery expands vector bases into x/y/z component features and searches for sparse linear combinations via a GA (with an optional parsimony penalty).
\item Sparse-regression solvers (e.g., STLS/LASSO) are promising alternatives for estimating coefficients on a fixed library, but are not yet implemented in the current Rust codebase.
\end{itemize}
