...
\begin{itemize}
\item \textbf{structured\_basic} (current implementation): direction-preserving, component-wise bases that mirror the analytic force contributions \emph{without} physical constants:
\begin{itemize}
...
\item \textbf{structured\_extended} (current implementation): adds a small number of carefully chosen atoms to expand expressivity while preserving interpretability, including (i) additional distance scalings (e.g., $1/r^4$ variants), and (ii) simple distance-based gating (``close'' vs.\ ``far'' features) to help isolate regime-specific structure without resorting to an unconstrained polynomial soup.
\end{itemize}
...
At the end of a run, we additionally render reproducible evaluation artifacts (Markdown and LaTeX) that record the best discovered equations (exact text), the best-run initial conditions, and a best-vs-history comparison over prior local attempts. The global best-results ledger is also updated to capture the current incumbent per simulation bucket.
...
Current implementation outputs CSV and a JSON ``sidecar'' containing config, header hash, warnings, last-regime diagnostics, dt statistics (accepted/rejected/min/max/avg), and the initial state (masses/charges/positions/velocities) to support strict reproducibility.
